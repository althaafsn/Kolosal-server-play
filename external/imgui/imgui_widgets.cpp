...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_enter_pressed || is_gamepad_validate)
        {
            ...
            bool shift_enter_for_new_line = (flags & ImGuiInputTextFlags_ShiftEnterForNewLine) != 0;
            if (!is_multiline || is_gamepad_validate || (ctrl_enter_for_new_line && !io.KeyCtrl && !io.KeyShift) || (!ctrl_enter_for_new_line && io.KeyCtrl && !io.KeyShift) || (shift_enter_for_new_line && !io.KeyShift && !io.KeyCtrl) || (!shift_enter_for_new_line && io.KeyShift && !io.KeyCtrl))
            {
                validated = true;
